...
export default function AdminRegistrationsPage() {
    ...
    const fetchRegistrations = async () => {
      try {
        ...
        const registrationsData = snapshot.docs.map((doc) => ({
          ...
          ...doc.data(),
          fechaInscripcion:
            doc.data().fechaInscripcion?.toDate?.() || new Date(),
          fechaNacimiento: formatDate(doc.data().fechaNacimiento) || "-",
        }));

        const years = [
          ...new Set(
            registrationsData.map((reg) => reg.fechaInscripcion.getFullYear())
          ),
        ];
  ...
  useEffect(() => {
    ...
    if (yearFilter !== "all") {
      filtered = filtered.filter(
        (reg) => reg.fechaInscripcion.getFullYear() === parseInt(yearFilter)
      );
    }
  ...
  const sendConfirmationEmail = async (participant) => {
    if (!participant || !participant.email) {
      console.error(
        "No se puede enviar email: información del participante incompleta"
      );
      return false;
    }
    ...
    try {
      const templateParams = {
        ...
        fechaInscripcion:
          participant.fechaInscripcion.toLocaleDateString("es-ES"),
      };
  ...
  const updateRegistrationStatus = async () => {
    ...
    try {
      ...
      if (
        ...
        selectedRegistration.estado !== "confirmado"
      ) {
        ...
        prev.map((reg) =>
          reg.id === selectedRegistration.id
            ? {
                ...
                fechaActualizacion: new Date(),
                emailEnviado:
                  newStatus === "confirmado" ? true : reg.emailEnviado,
              }
            : reg
        )
      );
  ...
  const exportApprovedToPDF = () => {
    ...
    <body>
      ...
      <div class="summary">
        <p>Total de participantes confirmados: <strong>${
          ...
          ${approvedRegistrations
            .map((reg, index) => {
              ...
              if (reg.condicionSalud) {
                ...
                if (
                  typeof reg.condicionSalud === "string" &&
                  reg.condicionSalud.trim().startsWith("{")
                ) {
                  try {
                    const parsed = JSON.parse(reg.condicionSalud);
                    condicionesDeSalud =
                      parsed.condicionSalud || parsed.condicionesSalud || "";
                  } catch (e) {
    ...
    link.setAttribute(
      "download",
      `participantes_confirmados_${new Date().toISOString().split("T")[0]}.html`
    );
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      default:
        return (
          <Badge className="bg-gray-500 hover:bg-gray-600">Pendiente</Badge>
        );
    }
  };
  const getStatistics = () => {
    const total = filteredRegistrations.length;
    const confirmados = filteredRegistrations.filter(
      (reg) => reg.estado === "confirmado"
    ).length;
    const pendientes = filteredRegistrations.filter(
      (reg) => reg.estado === "pendiente" || !reg.estado
    ).length;
    const rechazados = filteredRegistrations.filter(
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col items-center justify-center mt-20 mb-8 px-4">
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            ...
            <div className="flex gap-2">
              ...
              <Button
                ...
                disabled={
                  registrations.filter((r) => r.estado === "confirmado")
                    .length === 0
                }
              >
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-10xl0 mx-auto">
                ...
                <div>
                  ...
                  <p className="text-sm">
                    {selectedRegistration.localidad || "-"}
                  </p>
                </div>
                <div>
                  <Label className="text-sm font-medium text-gray-500">
                ...
                <div>
                  ...
                  <p className="text-sm uppercase">
                    {selectedRegistration.talleRemera || "-"}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <p className="text-sm">
                    {selectedRegistration.fechaInscripcion?.toLocaleDateString(
                      "es-ES"
                    ) || "-"}
                  </p>
                </div>
                <div>
                  ...
                  <p className="text-sm">
                    {selectedRegistration.grupoSanguineo || "-"}
                  </p>
                </div>
                <div>
                  <Label className="text-sm font-medium text-gray-500">
                ...
                <div>
                  ...
                  <p className="text-sm">
                    {selectedRegistration.grupoCiclistas || "-"}
                  </p>
                </div>
              </div>
                ...
                <div>
                  ...
                  {loadingComprobante ? (
                    ...
                    <div className="border rounded-md p-2">
                      {selectedRegistration.comprobantePagoUrl?.endsWith(
                        ".pdf"
                      ) ||
              ...
              >
                ...
                >
                  <img
                    ...
                    className={`${
                      zoomedImage ? "cursor-move" : "cursor-zoom-in"
